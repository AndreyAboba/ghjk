...
local Config = {
	...
	-- trueGap - X, где X ≈ 75мс. Тогда всё сходится: 87→12мс (вн����������три окна, но у
	-- самого ��р��я), 106→31мс, а провалы 68→-7мс и 59���-16мс — блок пришёл ПОСЛЕ
	-- удара. Механизм X разложим:
	--   ~38мс — uplink вычитает RTT*0.5 (up=83) при истинном одн��стороннем ~45мс;
	...
	-- ⚠⚠ НЕ КАЛИБРОВАТЬ ПО trueGap. Рецепт в следующих двух строках ОШИБОЧЕН и
	-- оставлен только чтобы его узнавали: он даёт правдоподобный, но ложный ответ.
	-- Ловушка сработала уже ТРИЖДЫ (последний раз — на логе 2026-08, где по
	-- trueGap «рабочей зоной» выглядели 100-150мс и предлагалось поднять lead
	-- 62 → 110мс). Перед любым тюнингом lead проверяй ТОЛЬКО predErr по исходу:
	--   тот же лог, PERFECT n=26 predErr медиана +4мс | LATE n=32 медиана -3мс
	-- → НЕОТЛИЧИМО, тайминг нажатия не разделяет исходы. Ещё признак подделки:
	-- corr(trueGap, predErr) = +0.59, т.е. trueGap двигается вместе с `measured`,
	-- а не с нашим прицелом. Если правка обоснована только trueGap — она подгонка.
	...
	-- ═══ ОФСЕТ ОБНУЛЁН: 75 → 40 → 0. НЕ ПОДНИМАТЬ БЕЗ НОВОГО ДОКАЗАТЕЛЬСТВА. ═══
	-- Весь разбор выше построен на trueGap, а trueGap считается от серверного
	-- события исхода — то есть от следствия нашего же парри. Решающая проверка на
	-- логе V176 (34 нажатия, две метрики на ОДНИХ И ТЕХ ЖЕ событиях):
	--
	--   lateBy (аним-таймлайн, от исхода не зависит): PERFECT -48 | LATE -46  → 2мс
	--   trueGap (от серверного события исхода):       PERFECT 142 | LATE  87  → 55мс
	--
	-- Тайминг нажатия исходы НЕ РАЗДЕЛЯЕТ. «Зависимость» видна только в метрике,
	-- которую порождает сам исход. Офсет двигал РЕАЛЬНЫЙ момент нажатия, чтобы
	-- подогнать ЛОЖНОЕ показание: lead уезжал 62 → 109мс, и скрипт жал на ~47мс
	-- раньше нужного. Это и есть та «поздняя реакция», на которую жаловался юзер:
	-- гард вставал заранее, к контакту окно парри уже истекало, удар шёл в блок.
	--
	-- Правильное значение lead берётся из конфига ИГРЫ, а не из подгонки по логу:
	-- окно перфект-блока = [contact-0.125 .. contact], центр = 62.5мс → PerfectLead
	-- 0.0625. Ноль здесь означает «целимся в центр окна», и это проверяемо.
	ServerGapOffsetMs = 0,
	-- Диагностика оставлена, но ТОЛЬКО как показание. По ней ничего не настраивать:
	-- граница PERFECT/LATE по trueGap — это ровно тот ложный сигнал, что разобран
	-- выше. Для оценки тайминга смотреть lateBy из TRACE-PRESS.
	GapCalibDiag  = true,
	...
	-- КОРЕНЬ БАГА "блок вмест��� парри": track.Speed у ЧУЖОЙ анимации на нашем
	...
	-- ЗДЕСЬ БЫЛ БЛОК ResidComp* (прогрев, шринк, капы, окно усреднения) — настройки
	-- обучаемого сдвига прогноза контакта. Удалён вместе с самим обучателем.
	-- Сохранён один факт из того разбора, он важен и без обучения: predErr вокруг
	-- своего среднего гуляет на ±150мс (у tutelzs подряд +64,-46,+31,-15,-83,+159,
	-- -107,-150 при среднем ~+30мс) — при окне парри 125мс это значит, что НИКАКОЕ
	-- среднее по этой величине не даёт пригодной поправки. Ключи удалены целиком,
	-- чтобы не осталось тумблера, который можно включить «на посмотреть».
	...
	-- ВНИМАНИЕ: 0.18 — НЕ таймаут подтверждения рывка. Этот комментарий уже дважды
	-- отправлял разбор в ложном направлении («ждём ack 1209мс вместо 180мс — баг»).
	-- Проверено по дампу, Evasive_ModuleScript:739 — игра ждёт так:
	--     local v69 = granted and 1.2 or math.max(ServerConfirmTimeout, 0.6)
	-- то есть 1.2с при выданных i-frames и не меньше 0.6с иначе; 0.18 поднимается
	-- до 0.6 через math.max и в роли таймаута не участвует НИКОГДА. Наш код на
	-- ~5410 повторяет игровую формулу дословно, и это правильно: снижать ожидание
	-- до 0.18 значит разойтись с игрой и считать живые подтверждения потерянными.
	-- Константа ниже НЕ ЧИТАЕТСЯ нигде в скрипте и оставлена только как справка
	-- о значении из ClientPredict. Не использовать как окно ожидания ack.
	EvasiveAckTimeout  = 0.18, -- справочно (game: ClientPredict.Evasive.ServerConfirmTimeout)
	...
	-- ── ВСЯ АВТОКАЛИБРОВКА ВЫРЕЗАНА. ТУМБЛЕРОВ БОЛЬШЕ НЕТ. ────────────────────
	-- Раньше здесь стояли GapBiasComp/ResidComp = false и их параметры. Ключи
	-- удалены вместе с кодом обучателей (см. onOutcome и место их определения),
	-- чтобы не осталось ничего, что можно включить «на попробовать».
	-- Причина в одном предложении: единственные сигналы, доступные для обучения
	-- (trueGap и predErr), вычисляются от события, которое порождает наш же исход,
	-- а остаточная ошибка двусторонняя (±150мс при окне 125мс). Подгонка по такому
	-- сигналу улучшает лог и ухудшает игру.
	-- Проверка на числах лога показала это прямо: поправка -27мс
	...
	-- Цена ошибок разная и это ЕДИНСТВЕННОЕ, что стоит помнить из всего разбора:
	-- слишком рано = обычный блок (урона нет), слишком поздно = ПРОПУЩЕННЫЙ УДАР.
	-- Поэтому пол зазора ниже остаётся, а обучаемый кап (GapBiasCapMs) удалён.
	...
	-- Аварийное нажатие, когда хитбокс уже физически пере��екается с нами, а
	-- запланированный момент нажатия ещё в будущем (поздно оп��знанный свинг).
	-- Разбор случая — у самого условия в резолв��ре.
	EmergencyPress      = true,
	...
	-- Т.е. после снятия блока повторное парри доступно только через 0.5с — поэтом����
	-- два у��ара с разрывом меньше ~0.62с парировать ОБА физически нел��зя, и
...
--   PostureResistanceMultiplier = 0.9    (-10% урона по стойке = д��льше держит гард)
...
local Threats = {}
	...
	-- ЗДЕСЬ БЫЛИ ResidByAtk / ResidGlobal / GapBias — накопители автокалибровки.
	-- Удалены вместе с обучателями: копить статистику, на которую ничто не
	-- опирается, значит держать наготове соблазн её применить. ResidByKS выше
	-- оставлен, он питает ТОЛЬКО показание resAvg в диаге.
local ComboState = {}
local Pending = {}
...
local function canBlockNow()
	...
	if stunned or cantAny then
		-- Условие сверено с КОДОМ ИГРЫ (Block_ModuleScript.Block, ветка буферного
		-- парри). Игра в состоянии Stunned/CantAnything сама отправляет Activated,
		-- если: ParryWindowDisabled ~= true, ParryBuffered == true,
		-- GuardBroken ~= true, PerfectBlocking ~= true. То есть стан — это НЕ запрет
		-- блока, а ровно тот случай, когда парри и надо жать.
		-- Раньше эта ветка дополнительно требовала `Config.ComboEscape`, и при
		-- выключенном флаге мы отказывались парировать при ParryBuffered=true —
		-- в логе V176c это видно как `SELF-LOCK ... ParryBuffered=true`, после чего
		-- стан-каскад съедал серию ударов. Игра такого условия не имеет, убрано.
		if c:GetAttribute("ParryWindowDisabled") ~= true
		   ...
		   and (c:GetAttribute("Parried") == true
			or c:GetAttribute("ParryBuffered") == true) then
			return true, nil
		end
...
-- Доворот раньше шёл по сырому th.threatens, а этот флаг намере��но «щедрый»:
-- в него попадают свинги, оставленные включёнными geom-латчем (то есть те, где
-- живая геометрия УЖЕ отказала, а решен��е держится по инерции от прошлых кадров).
...
local willHitMe = LPH_NO_VIRTUALIZE(function(th)
	...
	if mode == "High" then
		...
		-- оставшегося времени и скорости поворота, а не берём конст��нтой.
		...
		-- враги с dist=9..10 проходили как угроза. Здесь нужен тол��ко допуск на
		-- ошибку нашей реконструкции бокса (ProvenReachPad, ~2 стада), а
		-- сближение уже у��тено отдельно в approachAllow.
...
local function loadGameModules()
	...
	-- Критический модуль — CombatConfig: без него мертвы и окно парирования, ��
...
-- `if s < 0 then s = 0 end`, то есть ум��ет только ОТОДВИГАТЬ прогноз позже.
...
--
-- Старая цепочка ист��чников роста обрывалась на двух пунктах, и оба часто
...
local function resolveCharHeight(model)
	...
	if not h and Config.HeightDeepFallback ~= false then
		...
		-- у любо��о отрендеренного персонажа. R6 не скалится ростом — там 1.0.
		pcall(function()
			local hum = model:FindFirstChildOfClass("Humanoid")
...
local resolveInfo = function(id, model)
	...
	return {
		...
		-- styleKey() объявлен НИЖЕ этой функции, поэтому нормализуем на ��есте.
...
-- Н��рмализация ключа стиля для LEGACY-таблиц.
...
-- ────────────────────────────���─────────────────────────────────────────────────
-- Игра (M1_ModuleScript / M2_ModuleScript) ставит хит��окс так:
...
local function hitTimeline(info, combo, mult, contactBase)
	...
	-- И��ра: GetScaledHitboxDelay(d, m) = d/max(0.05,m) + extra/max(0.05,m)
...
-- ЗДЕСЬ БЫЛА residContactBias() — обучаемый сдвиг прогноза контакта по остатку
-- predErr. Удалена (см. onOutcome): predErr на PERFECT равен ~0 по построению,
-- потому что исход приходит нашей же сменой состояния в момент парри. Функция к
-- моменту удаления уже никем не вызывалась — в диаге это было видно как
-- `atkResid=+0ms bias=+0ms` в каждой строке OUT. НЕ ВОССТАНАВЛИВАТЬ.
...
-- центн��я: velLead>=24мс → guard=EARLY(>125), velLead<=8мс → guard=IN-WINDOW.
--
...
-- ЗДЕСЬ БЫЛИ gapBiasEst / gapBiasLearn — обучаемая невязка серверного зазора.
-- Удалены вместе с ResidComp: разбор в onOutcome. Коротко: единственный сигнал,
-- на котором они могли учиться (trueGap / predErr), порождён исходом, который мы
-- же и создаём, а сама ошибка двусторонняя. НЕ ВОССТАНАВЛИВАТЬ.
...
function V93.markOwnM2IFrames(now, tag)
	...
	-- мо��ча теряло второй контакт нашей связки при расчёте counter i-frames.
...
local function fireBoxingCounter(th, targetDist)
	...
	tx.expectedIFramesAt = (cs == "wingchun") and math.huge
			...
			-- захардкоженные 16.7мс занижа��и запас в 3.4 раза.
...
local onAttack = function(attackerHRP, info, model, id, track)
	...
	if dist > reachMax then
		...
		pcall(function()
			...
			if flat.Magnitude > 0.1 then
				...
				-- отрицательным → math.max(...,0) обнулял запас именн�� тем, кто бежит
	...
	-- info.contacts (мультихит Boxing M2) — сырые аним-метки, ��х надо поделить на
	-- aMult самим, иначе мультихит окажется в других един��цах, чем обычный удар.
...
local schedulerStep = LPH_NO_VIRTUALIZE(function(now)
	...
	V93.nearPressStamp = os.clock()
		...
		elseif not th.dodged then
			...
			-- как свой до кон��а. Отсюда `off-target swings rejected=1` на 243
			...
			if Config.SkillAddon and Config.SA_HakariRead and th.kind == "M2"
				...
				hold = hold + w
					...
					local pwin = (Config.PerfectWindowLive ~= false and GameData.perfectWindow)
						...
						-- Жёсткий ноль вместо обучаемой невязки (см. разбор в onOutcome).
						-- Оставлено переменной, а не вырезано из формул, чтобы вся
						-- арифметика н��же читалась как раньше и никто не «починил»
						-- её обратно, не заметив, что слагаемое исчезло.
						local gapBias = 0
						local edgeS = (Config.GapEdgeMs or 6) / 1000
						local pmin0 = Config.PerfectMin or 0.05
						-- velLead тоже надо ��граничить окн��м. Иначе при большом реальном
						...
						-- иначе они режут прицел до старого, промахивающегося значения.
							-- Тот же физический сдвиг, что и у цели ниже (uplink). Если оставить
							-- здесь ноль, leadCap/vlCap обрежут lead обратно к промахивающемуся
							-- значению и правка цели не даст ничего.
							local up0 = uplink()
							if type(up0) ~= "number" or up0 ~= up0 then up0 = 0.06 end
							local gapOff0 = math.max(math.min(up0, 0.15), 0.02)
					...
					-- ── ЗАПАС НА КВА��ТОВАНИЕ КАДРОМ ────────────────────────────
						...
						-- ОПУСКАЛА lead (62→48мс при кадре 38мс). При задержке в кадр з��зор
						...
						-- обрезал бы цель обратно и правка не дала бы ничего.
							-- ═══ ФИЗИЧЕСКИЙ СДВИГ ЦЕЛИ = ИЗМЕРЕННЫЙ UPLINK ═══════════════
							-- expGap измеряется в КЛИЕНТСКОМ времени: press → contact. Но сервер
							-- поднимает гард не в момент нажатия, а когда ДОЕДЕТ пакет, т.е. на
							-- press+uplink. Серверный зазор = expGap - uplink, и парри существует
							-- только если contact попал в [активация, активация+окно]:
							--     expGap ∈ [uplink, uplink + pwin]   → центр = uplink + pwin/2
							-- Раньше здесь стоял ServerGapOffsetMs (75, потом 40, потом 0) — это
							-- были кривые приближения uplink подгонкой по логам. Ноль означал
							-- «пакет доезжает мгновенно», отсюда обвал точности до 33%.
							-- Доказательство на логе V176c (11 разменов, БЕЗ trueGap):
							--     PERFECT expGap = [81, 87, 90, 94, 100]  мин 81
							--     LATE    expGap = [29, 43, 44, 59, 60, 62] макс 62
							-- Пересечений НЕТ, порог 62↔81 совпадает с uplink 60-76мс. При целе
							-- 70мс почти все размены падали ниже порога → LATE.
							-- uplink() causal: считается из сетевой статистики, а не из исхода,
							-- поэтому это НЕ запрещённая автокалибровка по trueGap.
							local upNow = uplink()
							if type(upNow) ~= "number" or upNow ~= upNow then upNow = 0.06 end
							-- Санитарный кап: при выбросе пинга не уводим цель за окно.
							local gapOff = math.max(math.min(upNow, 0.15), 0.02)
	...
	if not clusterStrategy and Config.MultiThreatGuard and clusterN >= (Config.MultiThreatMinN or 2) then
			...
			if anchor and not anchor.planCovered then
				...
				elseif Config.AutoDodge then
					...
					for j = i + 1, clusterN do
						...
						-- ПЕРФ: см. комментар��й у ветки парри — строка только для лога.
						if Config.DeepDiag then
							planParts = (planParts and planParts .. ", " or "")
		...
		-- а planParts СОДЕРЖИТ «+%.0fms до контакта» — величину, которая меняет��я
		...
		-- Теперь подпись из СТАБИЛЬНЫХ поле��: стратегия и состав плана.
	...
	if dodgeReady() and canDodgeNow() and #imminent >= 1 then
		...
		if multiNow and not coverable then
			-- Додж един и накрывает ВСЁ, что попадае�� в окно неуязвимости, поэтому
	...
	-- остаётся на реаль��ой цели, а не прыгает на чужие свинги.
	...
	if wantBlock and State.interruptFiredFrame == _C.FrameId then wantBlock = nil end
		...
		-- перекрывается текущи�� нажатием. Всё, что дальше, обрабатывается ��воим
		-- решением в следующих ка��рах.
		-- ОКНО ПАРРИ ИДЁТ ОТ НАЖАТИЯ, А НЕ ОТ ТЕКУЩЕГО КАДРА.
		-- Здесь стояло `th.contactAbs <= now + heldPwin + up`, и это завышало
		-- покрытие ровно на (now - lastPress). Игра считает perfect-block от
		-- момента активации гарда (CombatConfig.Block.PerfectBlockWindow = 0.125
		-- от Blocking=true), поэтому уже поднятый гард закрывает контакты только
		-- до lastPress + 0.125, а не до now + 0.125. Чем дальше кадр от нажатия,
		-- тем больше врал старый тест: при гарде, поднятом 100мс назад, он
		-- объявлял «накрыто» удар, который придёт через 125мс, хотя от нажатия
		-- до него уже 225мс — вне окна, и игра засчитает ОБЫЧНЫЙ БЛОК.
		-- Ровно это видно у быстрых (низкорослых) врагов: их серия идёт с
		-- интервалом ~0.46с при нашем BlockCooldown 0.5с, повторно нажать нельзя,
		-- и удары молча уходили в held-guard как «обработанные» — то есть в блок
		-- вместо парри, при 0% таких блоков у высоких врагов в тех же логах.
		-- Помечать угрозу pressed можно только если её реально накрывает окно
		-- текущего нажатия; всё остальное должно получить своё решение (додж).
		...
		local function heldGuardCovers(th)
			...
			-- Без известного момента нажатия покрытие не заявляем: пусть удар
			-- обработается своим решением, чем будет тихо списан в «накрыт».
			...
			if clusterStrategy == "HELD_GUARD" and State.blocking then
				for _, th in ipairs(cluster) do
					...
					elseif Config.DeepDiag and not th.heldMissLogged then
						-- Видимость главного симптома: удар, который раньше молча
						-- уходил в зажатый гард (= обычный блок вместо парри).
						-- sincePress > окна означает, что нажатие уже «выдохлось».
		...
		-- ══ ОТСРОЧКА НАЖАТИЯ РАДИ ТЯЖЁЛОГО УДАРА ═══════════════════════════
			...
			-- лёгкий удар, а тяжёлый прилетит ��нутри кулдауна после него — нажатие
		...
		local base = wantBlock.contactAbs
			...
			-- или додж). Раньше исключался то��ько SEQUENTIAL.
	...
	elseif State.blocking then
		...
		-- равно отпускал гард — удар приходи�� чистым:
...
local function onOutcome(attacker, result, kind, eventClock)
	...
	-- ЗДЕСЬ БЫЛА МОЯ ОШИБОЧНАЯ ПРАВКА (гипотеза «залипшего гарда») — ОТКАЧЕНА.
	-- Я утверждал, что гард остаётся зажатым и повторный Activated не создаёт
	-- perfect-окна. Лог V176c это опроверг напрямую: в 15 нажатиях из 15 гард перед
	-- нажатием был ОПУЩЕН, а сервер исправно выставлял PerfectBlocking. Заодно
	-- рассыпалось моё «LATE = урон сквозь блок»: в сводке BLOCK=0, то есть блоков
	-- не было вовсе — LATE это чистый пропущенный удар.
	-- Не добавлять сюда releaseBlock: настоящая причина промахов была в цели
	-- зазора (см. правку gapOff = uplink), а лишнее отпускание только плодило
	-- отказы `refused: BlockCooldown`.
	...
	-- ═══ ОБУЧЕНИЕ УДАЛЕНО НАВСЕГДА. НЕ ВОЗВРАЩАТЬ. ═══
	-- Здесь копился per-attacker остаток predErr и применялся к contactAbs.
	-- Причина удаления не «не успели настроить», а то, что сигнал НЕПРИГОДЕН:
	--
	--   predErr = measured - contact, где measured = (момент исхода - детект).
	--   На PERFECT исход приходит нашей же сменой состояния в момент парри, т.е.
	--   measured ≈ contact ПО ПОСТРОЕНИЮ. На промахе исход приходит иначе.
	--   Проверено на логе V176: |predErr| = 14мс на PERFECT против 87мс на LATE.
	--   Обучаться на этом = обучаться на собственном исходе, а не на тайминге.
	--
	-- Вторая, независимая причина: ошибка ДВУСТОРОННЯЯ. В том же логе у промахов
	-- predErr = [-110,-83,-81,+27,+76,+87,+93,+135,+157] — 3 в минус, 6 в плюс.
	-- Одно скользящее среднее принципиально не описывает двусторонний разброс:
	-- любой сдвиг, который лечит одну половину, ровно настолько же ломает другую.
	--
	-- resAvg ниже остаётся, но ТОЛЬКО как показание в диаге (ResidByKS). Ни на
	-- один тайминг он не влияет и влиять не должен.
	...
	-- ═══ ОБУЧЕНИЕ ПО trueGap УДАЛЕНО НАВСЕГДА. НЕ ВОЗВРАЩАТЬ. ═══
	-- Здесь стоял gapBiasLearn(trueGap - aimGap). trueGap — САМАЯ ядовитая метрика
	-- в этом скрипте, потому что выглядит убедительно и врёт. Решающая проверка на
	-- логе V176 (34 нажатия), две метрики на одних и тех же событиях:
	--
	--   по lateBy (аним-таймлайн, от исхода НЕ зависит):
	--       PERFECT -48мс | LATE -46мс  → разница 2мс, НЕОТЛИЧИМО
	--   по trueGap (считается от серверного события исхода):
	--       PERFECT 142мс | LATE  87мс  → разница 55мс, «явная зависимость»
	--
	-- Первая метрика говорит, что тайминг нажатия исход не определяет. Вторая
	-- рисует сильную связь — потому что событие, от которого её считают, само
	-- порождено исходом. Любая настройка по trueGap подгоняет скрипт под
	-- собственное следствие; именно так появился ServerGapOffsetMs=40, поднявший
	-- lead 62→109мс. Ловушка сработала ЧЕТЫРЕ раза.
	--
	-- trueGap ниже остаётся только для печати в диаге.
	...
	-- atkResid/bias убраны из строки: их источники удалены вместе с автокалибровкой,
	-- и печатать вечные +0ms значит намекать, что там есть работающий механизм.
